...
namespace DiverseCrowd
{
    ...
    {
        private const string HarmonyId = "github.com/bloodypenguin/Skylines-MoreDiverseCrowd";
            ...
            MethodDefinition transpiler = null)
        {
            ...
            {
                throw new Exception(
                    $"MoreDiverseCrowd: prefix, postfix and transpiler are null for method {original.Type.FullName}.{original.MethodName}");
            }

            try
            {
                Debug.Log($"MoreDiverseCrowd: Patching method {original.Type.FullName}.{original.MethodName}");
            ...
            {
                Debug.LogError($"MoreDiverseCrowd: Failed to patch method {original.Type.FullName}.{original.MethodName}");
        ...
        {
            Debug.Log($"MoreDiverseCrowd: Unpatching method {original.Type.FullName}.{original.MethodName}");
        ...
        private static MethodInfo GetOriginal(MethodDefinition original)
        {
            ...
            {
                throw new Exception(
                    $"MoreDiverseCrowd: Failed to find original method {original.Type.FullName}.{original.MethodName}");
            }
        ...
        private static MethodInfo GetPatch(MethodDefinition patch)
        {
            ...
            if (methodInfo == null)
            {
                throw new Exception($"MoreDiverseCrowd: Failed to find patch method {patch.Type.FullName}.{patch.MethodName}");
            }
